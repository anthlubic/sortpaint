...
func _ready() -> void:
	# LOCAL EDIT (not upstream): the command port stays off in exported release builds.
	# Upstream listens unconditionally, which would ship a localhost eval endpoint to players.
	# Re-apply this guard whenever this file is re-copied from godot-mcp.
